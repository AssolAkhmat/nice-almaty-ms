...
export const ORG_SETTINGS = {
   ...
   * (указание владельца, 22 сентября 2026). Пусто — подписи нет, и договор
   * с токеном `{{owner.signature}}` не собирается вовсе: отказ называет
   * причину (уточнение владельца от 26 сентября 2026). Прежде на месте
   * токена оставалась пустота, и документ выходил с пустым местом там,
   * где должна стоять подпись.
   *
