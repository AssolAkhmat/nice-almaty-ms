...
export interface UploadMeta {
  ...
  sizeBytes: number;
  /**
   * Origin страницы, из которой пойдут байты. Сессия прямой загрузки
   * привязывается к нему: без него ответ хранилища на `PUT` из браузера
   * не несёт CORS-заголовков, и браузер прячет его за `Failed to fetch`
   * (инцидент I11). Пусто — байты пришлёт сам сервер, и CORS ни при чём.
   */
