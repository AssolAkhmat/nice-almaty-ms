...
export async function identifyByToken(
  ...
  return {
    context: {
       ...
       * Сужение действует и для суперадмина: `visibleHouseIds` отдаёт ему
       * весь список домов только при пустом доме в контексте, а `can`
       * с областью `org` сверяет дом цели (`src/db/access.ts`, `src/lib/authz.ts`).
       * До 20 сентября 2026 обещание не выполнялось: токен суперадмина,
       * выданный на один дом, действовал на всю сеть.
       */
