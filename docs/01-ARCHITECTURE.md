...
Человек открывает документ ссылкой `GET /api/v1/files/{id}/view`: она проверяет права
и переводит на содержимое с **пятиминутным пропуском** (решение D25). Пропуск ничего
не разрешает сверх прав — он отнимает срок жизни у адреса, который остаётся в истории
браузера. Без пропуска содержимое человеку не отдаётся; токену API пропуск не нужен,
адресной строки у него нет. Документ показывается встроенным просмотрщиком браузера
(`Content-Disposition: inline`) и только для безопасных типов; `?download=1` скачивает.
