...
1. Клиент → `POST /api/v1/files/upload-session` (тип документа, mime, размер).
   ...
   и возвращает URL для прямой загрузки. Сессия у хранилища открывается
   с `Origin` страницы, которая пришлёт байты: сервер загрузки Google отдаёт
   CORS-заголовки на `PUT` по адресу сессии только названному origin, а без
   него браузер прячет ответ за `Failed to fetch` (инцидент I11).
...
APP_DOMAIN=                   # домен для caddy; сертификат, TLS и единственная дверь снаружи
BACKUP_AGE_RECIPIENT=         # открытый ключ age для шифрования дампов; закрытый у владельца
BACKUP_FOLDER_NAME=           # папка Drive под копии, отдельная от документов
BACKUP_WORK_DIR=              # каталог хоста для временного дампа
