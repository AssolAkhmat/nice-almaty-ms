...
- [x] `docker compose up` поднимает пустое приложение
- [x] Переключается тема (светлая / тёмная / системная)
- [x] Переключается язык (ru / kk / en)
- [x] `pnpm verify` зелёный
- [x] `GET /api/health` отвечает (БД и хранилище)
- [x] `next build` проходит с `DEPLOY_TARGET=vercel`
- [x] `docs/DEPLOY-VERCEL.md` существует и пригоден для превью-деплоя владельцем
...
- DoD: проверка на 375 / 768 / 1440, обе темы — закрыто e2e в T0.10

### T0.4 — Тема: светлая / тёмная / системная
- [x] Inline-скрипт в `<head>` против мигания (читает localStorage, ставит класс `.dark`)
- [x] `ThemeProvider` + хук, три режима, запись в `localStorage`
- [x] Компонент переключателя темы в шапке
- [x] Задел: синхронизация с профилем пользователя появится в фазе 1 (в БД ещё нет `users`)
...
- [x] Форматирование дат и денег через `next-intl` + `src/lib/time.ts`, зона `Asia/Almaty`
...
- [x] `src/env.ts` — zod-схема всех переменных из `01-ARCHITECTURE.md`, `import 'server-only'`, без значений по умолчанию для секретов
- [x] Условная валидация: `GDRIVE_*` обязательны только при `STORAGE_DRIVER=gdrive`, `SUPABASE_*` — при `supabase`
- [x] Проверка `FIELD_ENCRYPTION_KEY` — ровно 32 байта base64
- [x] `.env.example` со всеми ключами и пустыми значениями секретов
- [x] `src/lib/logger.ts` — pino, JSON, поле `request_id`
- [x] Тесты схемы env (валидный набор, отсутствующий секрет, короткий ключ шифрования)
...
- [ ] При первом переключении на Supabase фактически проверить, что `btree_gist` ставится,
      ...
      — **перенесено**: Supabase в этой среде нет. Оба пути проверены на локальном
      `postgres:16` (владелец БД ставит расширение; роль без `CREATE` получает `WARNING`
      и нулевой код возврата). Порядок проверки — в `docs/DEPLOY-VERCEL.md`, шаг 2
...
- DoD: 375 / 768 / 1440, обе темы, цели нажатия ≥ 44×44 на мобильном — закрыто e2e в T0.10

### T0.9 — Каркас layout и адаптивная навигация
- [x] Десктоп (≥1024): постоянное боковое меню 240px
- [x] Планшет (768–1023): меню сворачивается в иконки
- [x] Мобильный (<768): нижняя навигация 4 пункта + «Ещё» (Sheet/бургер)
- [x] Шапка: переключатель темы, переключатель языка
- [x] Пункты меню — из типизированного конфига по 11 модулям; страницы-заглушки на `EmptyState`
- [x] Все подписи — ключи i18n в трёх локалях
- Коммит: `feat(ui): каркас layout и адаптивная навигация`

### T0.10 — Playwright и e2e приёмки фазы
- [x] `playwright.config.ts` (проекты: desktop 1440, tablet 768, mobile 375)
- [x] e2e: переключение темы сохраняется после перезагрузки
- [x] e2e: переключение языка меняет подписи интерфейса
- [x] e2e: навигация присутствует в трёх ширинах
- [x] Скрипт `test:e2e`
...
- [x] `README.md`: быстрый старт, обе цели развёртывания
- Коммит: `chore(build): сборка под цель vercel`

### T0.14 — Инструкция по превью-деплою на Vercel
- [x] `docs/DEPLOY-VERCEL.md`: создание проекта, привязка репозитория, build-команда
- [x] Полный список переменных окружения с пометкой обязательности на этапе фазы 0
- [x] Supabase: строка подключения через pooler (порт 6543, `?pgbouncer=true`) и
      `DIRECT_DATABASE_URL` (порт 5432) для миграций
- [x] `vercel.json`: заготовка секции `crons` с перечнем заданий из `01-ARCHITECTURE.md`
      (сами обработчики — фаза 6)
- [x] Порядок проверки после деплоя: `/api/health` → переключение темы → переключение языка
- [x] **Фаза не закрыта, пока этого файла нет**
- Коммит: `docs: инструкция по превью-деплою на vercel`

### T0.15 — Закрытие фазы
- [x] `docs/08-DECISIONS.md` дополнен решениями, принятыми по ходу фазы
- [x] `PROGRESS.md`: что сделано, что осталось, на чём споткнулся
- [x] `PROGRESS.md`: пометка, что kk-локаль нуждается в вычитке человеком до показа жильцам
- [x] Все чекбоксы выше закрыты
